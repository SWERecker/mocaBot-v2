...
def mirai_reply_text(target_id, session_key, text, friend=False):
    ...
    if(not target_id == '' and not session_key == '' and not text == ''):
        data_dict = {'sessionKey': session_key, 'target': target_id, 'messageChain': [{'type': 'Plain', 'text': str(text)}]}
        ...
        logging.info("[EVENT] reply_text => {m_type}, {target} : {content}".format(target=target_id,content=text,m_type=message_type))
...
def mirai_reply_image(target_id, session_key, path = '', image_id = '', friend=False):
    ...
    if(not target_id == '' and not session_key == ''):
        ...
        if(not friend):
            ...
            message_type = 'FRIEND'
            res = requests.post(url=config.friendMessage_url, data=final_data)
        logging.info("[EVENT] reply_image => {m_type}, {target} : path={p_path}, imageId={p_id}".format(target=target_id,p_path=path,p_id=image_id,m_type=message_type))
        r_json = json.loads(res.text)
        return r_json.get('code'), r_json.get('messageId')
    else:
        return 'error_invalid_parameter'
